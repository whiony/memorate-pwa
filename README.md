...
Notes, categories, preferences, and locally available photos remain accessible offline. Signing in automatically merges existing local notes into your private cloud collection. Changes sync after edits, when the app opens or returns to the foreground, and when connectivity returns. An open app checks for changes from other devices every 15 seconds.

Settings shows pending, syncing, offline, and error states. Conflicting edits are preserved as recovered copies, including drafts saved after another device has changed or deleted the note. Existing local collections are not cleared when signing in.
