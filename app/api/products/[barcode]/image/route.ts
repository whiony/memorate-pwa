...
export async function GET(request:Request,{params}:{params:Promise<{barcode:string}>}) {
 ...
 try {
  ...
  for(const url of [...new Set([product.imageUrl,...(product.imageUrls||[])])].slice(0,3)) {
   ...
   try {const bytes=await downloadProductImage(url);return new Response(bytes,{headers:{'Content-Type':imageMime(bytes)!,'Cache-Control':'public, max-age=86400','X-Content-Type-Options':'nosniff'}});}catch { /* A merchant image can expire; try another trusted provider image. */ }
  }
