...
import { relevantPrice } from '@/lib/product-info';
...
export async function GET(request:Request,{params}:{params:Promise<{barcode:string}>}) {
 ...
 try {const product=await productProvider().lookup(barcode,request.signal);return Response.json({product:product?{barcode:product.barcode,name:product.name,brand:product.brand,category:product.category,source:product.source,hasImage:!!product.imageUrl,productInfo:product.productInfo,categorySuggestion:product.categorySuggestion,suggestedPrice:relevantPrice(product.prices,currency)}:null},{headers:{'Cache-Control':'public, max-age=300'}});}
