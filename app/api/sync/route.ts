...
export const GET = (request: Request) => boundary(async()=>{
  ...
  const rows=await db.batch<Record<string,unknown>>([
    ...
    db.prepare("SELECT id,title,barcode,product_source AS productSource,product_info AS productInfo,comment,rating,price,currency,category_id AS categoryId,note_date AS date,created_at AS createdAt,updated_at AS updatedAt FROM notes WHERE owner_id = ? ORDER BY id").bind(owner),
  ...
  const notes=rows[2].results.map(n=>({...n,barcode:n.barcode || undefined,productSource:n.productSource || undefined,productInfo:typeof n.productInfo==="string"?JSON.parse(n.productInfo):undefined,photos:rows[3].results.filter(p=>p.noteId===n.id).map(({noteId:_noteId,...p})=>p)}));
...
export const POST = (request: Request) => boundary(async()=>{
  ...
  const statements=[
    ...
    db.prepare(`INSERT INTO notes (owner_id,id,title,comment,rating,price,currency,category_id,note_date,created_at,updated_at,barcode,product_source,product_info) SELECT ?,json_extract(value,'$.id'),json_extract(value,'$.title'),json_extract(value,'$.comment'),json_extract(value,'$.rating'),json_extract(value,'$.price'),json_extract(value,'$.currency'),json_extract(value,'$.categoryId'),json_extract(value,'$.date'),json_extract(value,'$.createdAt'),json_extract(value,'$.updatedAt'),json_extract(value,'$.barcode'),json_extract(value,'$.productSource'),json_extract(value,'$.productInfo') FROM json_each(?) WHERE ${guard}`).bind(owner,JSON.stringify(data.notes),owner,token),
