...
export const GET = (request: Request) => boundary(async()=>{
  ...
  const rows=await db.batch<Record<string,unknown>>([
    ...
    db.prepare("SELECT id,title,comment,rating,price,currency,category_id AS categoryId,note_date AS date,created_at AS createdAt,updated_at AS updatedAt FROM notes WHERE owner_id = ? ORDER BY id").bind(owner),
  ...
  const notes=rows[2].results.map(n=>({...n,photos:rows[3].results.filter(p=>p.noteId===n.id).map(({noteId:_noteId,...p})=>p)}));
...
export const POST = (request: Request) => boundary(async()=>{
  const {userId:owner}=await identity(request),{db,bucket}=database();
  const {revision,data}=payloadSchema.parse(JSON.parse(new TextDecoder().decode(await bytes(request,4*1024*1024))));
  ...
  for(const n of data.notes)for(const [i,p] of n.photos.entries()){const key=await photoKey(owner,p.id);if(!seen.has(key)){if(!(await bucket.head(key)))throw new ApiError(400,"Upload all photos before synchronizing.");seen.add(key);}photos.push({...p,noteId:n.id,sortOrder:i,storageKey:key,createdAt:n.createdAt,updatedAt:n.updatedAt});}
  ...
  const statements=[
    ...
    db.prepare(`INSERT INTO notes SELECT ?,json_extract(value,'$.id'),json_extract(value,'$.title'),json_extract(value,'$.comment'),json_extract(value,'$.rating'),json_extract(value,'$.price'),json_extract(value,'$.currency'),json_extract(value,'$.categoryId'),json_extract(value,'$.date'),json_extract(value,'$.createdAt'),json_extract(value,'$.updatedAt') FROM json_each(?) WHERE ${guard}`).bind(owner,JSON.stringify(data.notes),owner,token),
