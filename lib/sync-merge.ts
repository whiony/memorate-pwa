...
export function mergeSnapshots(base: Snapshot, local: Snapshot, remote: Snapshot, newId: () => string): { snapshot: Snapshot; conflicts: number } {
  ...
  for (const c of local.categories) {
    ...
    if (equal(c, b)) continue;
    if (r && !equal(r, b) && !equal(c, r)) { const id = newId(); categories.push({ ...c, id }); categoryMap.set(c.id, id); conflicts++; }
    else { const i = categories.findIndex(x => x.id === c.id); if (i >= 0) categories[i] = c; else categories.push(c); }
  }
  for (const b of base.categories) if (!local.categories.some(c => c.id === b.id) && equal(remote.categories.find(c => c.id === b.id), b)) { const i = categories.findIndex(c => c.id === b.id); if (i >= 0) categories.splice(i, 1); }
