...
function NoteCard({ note, category, onOpen }: { note: Note; category?: Category; onOpen: () => void }) {
  return <button className="note-card" onClick={onOpen} type="button"><div className="note-card-content"><div className="card-top"><span className="category-label"><span className="category-mark" style={{ background: category?.color || "#BCA9A2" }} />{category?.name || "Uncategorized"}</span><span className="card-date">{prettyDate(note.date)}</span></div><h2>{note.title}</h2>{note.comment && <p className="card-preview">{note.comment}</p>}<div className="card-bottom"><span className={`numeric-rating ${note.rating ? "" : "unrated"}`}>{note.rating ? <>{note.rating.toFixed(1)} <BrandStar className="rating-mark" /></> : "Not rated"}</span>{note.price != null && <span className="card-price">{money(note.price, note.currency)}</span>}</div></div>{note.photos[0] && <span className="card-photo-frame"><PhotoView blob={note.photos[0].blob} className="card-photo" alt="" /></span>}</button>;
}
