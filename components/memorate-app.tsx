...
function NoteEditor({ open, note, categories, currency, onClose, onSave, onAddCategory, onDelete }: {
  ...
  return <><Sheet open={open} onOpenChange={value => { if (!value) requestClose(); }}><SheetContent side="responsive" onDismiss={requestClose} showCloseButton={false} className="editor-sheet"><SheetHandle onClose={requestClose} /><div className="sheet-top"><button className="icon-button" onClick={requestClose} aria-label="Close"><X /></button><SheetTitle>{note ? "Edit note" : "New note"}</SheetTitle><span className="sheet-top-spacer" /></div><SheetDescription className="sr-only">Save a memory with a title and optional details.</SheetDescription>
