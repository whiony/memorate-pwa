...
function NoteEditor({ open, note, categories, currency, onClose, onSave, onAddCategory, onDelete, notes, onOpenExisting, initialProduct }: {
  ...
  return <>{open && scannerOpen && <BarcodeCapture notes={notes} onClose={()=>setScannerOpen(false)} onUse={useProduct} onExisting={existing=>{setScannerOpen(false);onOpenExisting(existing);}}/>}<Sheet open={open} onOpenChange={value => { if (!value) requestClose(); }}><SheetContent side="responsive" onDismiss={requestClose} showCloseButton={false} className="editor-sheet" onOpenAutoFocus={event => { event.preventDefault(); editorScroll.current?.scrollTo({top:0}); editorClose.current?.focus({preventScroll:true}); }}><SheetHandle onClose={requestClose} /><div className="sheet-top"><button ref={editorClose} className="icon-button" onClick={requestClose} aria-label="Close"><X /></button><SheetTitle>{note ? "Edit note" : "New note"}</SheetTitle><span className="sheet-top-spacer" /></div><SheetDescription className="sr-only">Save a memory with a title and optional details.</SheetDescription>
    <form ref={editorScroll} id="note-form" onChangeCapture={() => setDirty(true)} onSubmit={save} className="editor-scroll"><label className="field-label" htmlFor="note-title">Title <span className="required">*</span></label><input id="note-title" className="title-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="What do you want to remember?" required maxLength={160} />
      <div className="barcode-action"><button type="button" className="secondary-button" onClick={()=>setScannerOpen(true)}><Barcode size={20}/> Scan barcode</button>{barcode&&<span>Barcode: {barcode}</span>}</div>{productMessage&&<p className="product-message" role="status">{productMessage}</p>}
...
export default function MemorateApp() {
  ...
  <>{quickScan && <BarcodeCapture notes={notes} onClose={()=>setQuickScan(false)} onExisting={note=>{setQuickScan(false);openNote(note.id);}} onUse={product=>{openNewNote();setScannedProduct(product);setQuickScan(false);}}/>}</>
