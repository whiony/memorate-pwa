...
import { ArrowDown, ArrowLeft, ArrowUp, Camera, Check, ChevronDown, ChevronLeft, ChevronRight, Download, Ellipsis, Image as ImageIcon, Moon, Plus, Search, Settings2, SlidersHorizontal, Star, Sun, Trash2, X } from "lucide-react";
...
function NoteEditor({ open, note, categories, currency, onClose, onSave, onAddCategory, onDelete }: {
  ...
  const [photos, setPhotos] = useState<Photo[]>([]); const [queue, setQueue] = useState<File[]>([]); const [newCategory, setNewCategory] = useState(""); const [addingCategory, setAddingCategory] = useState(false); const [saving, setSaving] = useState(false);
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      await onSave({ id: note?.id || crypto.randomUUID(), title: title.trim(), rating, categoryId: categoryId || null, date, comment: comment.trim(), price: price === "" ? null : Number(price), currency: selectedCurrency, photos, createdAt: note?.createdAt || now, updatedAt: now, syncState: "local" });
      onClose();
    } catch { /* The parent shows a storage error; keep the editor open. */ }
    finally { setSaving(false); }
  }
  async function addCategory() { const category = await onAddCategory(newCategory); if (category) { setCategoryId(category.id); setNewCategory(""); setAddingCategory(false); } }
  return <><Sheet open={open} onOpenChange={value => { if (!value) onClose(); }}><SheetContent side="right" showCloseButton={false} className="editor-sheet"><div className="sheet-top"><button className="icon-button" onClick={onClose} aria-label="Close"><X /></button><SheetTitle>{note ? "Edit note" : "New note"}</SheetTitle><span className="sheet-top-spacer" /></div><SheetDescription className="sr-only">Save a memory with a title and optional details.</SheetDescription>
    <form id="note-form" onSubmit={save} className="editor-scroll"><label className="field-label" htmlFor="note-title">Title <span className="required">*</span></label><input id="note-title" className="title-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="What do you want to remember?" required maxLength={160} autoFocus />
      <div className="editor-section"><div className="section-heading"><span>Photos</span><span className="optional">Optional</span></div><input ref={inputRef} type="file" accept="image/*" multiple hidden onChange={e => { setQueue(prev => [...prev, ...Array.from(e.target.files || [])]); e.target.value = ""; }} /><div className="photo-strip">{photos.map((photo, index) => <div className="photo-edit" key={photo.id}><PhotoView blob={photo.blob} alt={`Photo ${index + 1}`} /><button type="button" className="photo-remove" onClick={() => setPhotos(prev => prev.filter(p => p.id !== photo.id))} aria-label={`Remove photo ${index + 1}`}><X size={14} /></button><div className="photo-move">{index > 0 && <button type="button" onClick={() => setPhotos(prev => { const next = [...prev]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; return next; })} aria-label="Move photo left"><ArrowLeft size={15} /></button>}{index < photos.length - 1 && <button type="button" onClick={() => setPhotos(prev => { const next = [...prev]; [next[index], next[index + 1]] = [next[index + 1], next[index]]; return next; })} aria-label="Move photo right"><ChevronRight size={15} /></button>}</div></div>)}<button type="button" className="add-photo" onClick={() => inputRef.current?.click()}><Camera size={22} /><span>Add photos</span></button></div></div>
      ...
      <div className="form-grid"><div className="form-field"><label className="field-label">Category</label><FieldSelect label="Category" value={categoryId || "none"} onChange={v => setCategoryId(v === "none" ? "" : v)} choices={[{ value: "none", label: "No category" }, ...categories.map(c => ({ value: c.id, label: c.name }))]} /><button type="button" className="inline-link" onClick={() => setAddingCategory(!addingCategory)}><Plus size={15} /> New category</button></div><div className="form-field"><label className="field-label" htmlFor="note-date">Date</label><input id="note-date" type="date" className="field-input" value={date} onChange={e => setDate(e.target.value)} required /></div></div>
      {addingCategory && <div className="inline-create"><input value={newCategory} onChange={e => setNewCategory(e.target.value)} placeholder="Category name" maxLength={32} onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); addCategory(); } }} aria-label="New category name" /><button type="button" onClick={addCategory} disabled={!newCategory.trim()}>Add</button></div>}
    ...
    </form><div className="editor-footer"><button type="submit" form="note-form" className="primary-button full" disabled={!title.trim() || saving}>{saving ? "Saving…" : note ? "Save changes" : "Save note"}</button></div>
  </SheetContent></Sheet><CropDialog file={queue[0] || null} onDone={blob => { setPhotos(prev => [...prev, { id: crypto.randomUUID(), blob }]); setQueue(prev => prev.slice(1)); }} onSkip={() => setQueue(prev => prev.slice(1))} /></>;
}

function NoteCard({ note, category, onOpen }: { note: Note; category?: Category; onOpen: () => void }) {
  return <button className="note-card" onClick={onOpen} type="button"><div className="note-card-content"><div className="card-top"><span className="category-label"><span className="category-mark" style={{ background: category?.color || "#CBD0D3" }} />{category?.name || "Uncategorized"}</span><span className="card-date">{prettyDate(note.date)}</span></div><h2>{note.title}</h2>{note.comment && <p className="card-preview">{note.comment}</p>}<div className="card-bottom"><Rating value={note.rating} />{note.price != null && <span className="card-price">{money(note.price, note.currency)}</span>}</div></div>{note.photos[0] && <PhotoView blob={note.photos[0].blob} className="card-photo" alt="" />}</button>;
}
...
export default function MemorateApp() {
  ...
  return <div className="app-shell"><div className="app-container"><header className="app-header"><button className="brand" onClick={() => { setSelectedId(null); setSettings(false); }} aria-label="Memorate home"><span className="brand-symbol" aria-hidden="true">m<span>·</span></span><span className="wordmark">memorate<span>.</span></span></button><button className={`icon-button settings-button ${settings ? "active" : ""}`} onClick={() => { setSettings(!settings); setSelectedId(null); }} aria-label="Settings"><Settings2 size={21} /></button></header>
