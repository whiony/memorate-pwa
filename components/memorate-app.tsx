...
function NoteEditor({ open, note, categories, currency, onClose, onSave, onAddCategory, onDelete, notes, onOpenExisting }: {
  notes: Note[]; onOpenExisting: (note: Note) => void; open: boolean; note: Note | null; categories: Category[]; currency: string; onClose: () => void;
  ...
  return <>{open && scannerOpen && <BarcodeCapture notes={notes} onClose={()=>setScannerOpen(false)} onUse={useProduct} onExisting={existing=>{setScannerOpen(false);onOpenExisting(existing);}}/>}<Sheet open={open} onOpenChange={value => { if (!value) requestClose(); }}><SheetContent side="responsive" onDismiss={requestClose} showCloseButton={false} className="editor-sheet" onOpenAutoFocus={event => { event.preventDefault(); editorScroll.current?.scrollTo({top:0}); editorClose.current?.focus({preventScroll:true}); }}><SheetHandle onClose={requestClose} /><div className="sheet-top"><button ref={editorClose} className="icon-button" onClick={requestClose} aria-label="Close"><X /></button><SheetTitle>{note ? "Edit note" : "New note"}</SheetTitle><span className="sheet-top-spacer" /></div><SheetDescription className="sr-only">Save a memory with a title and optional details.</SheetDescription>
    <form ref={editorScroll} id="note-form" onChangeCapture={() => setDirty(true)} onSubmit={save} className="editor-scroll"><label className="field-label" htmlFor="note-title">Title <span className="required">*</span></label><input id="note-title" className="title-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="What do you want to remember?" required maxLength={160} />
      <div className="barcode-action"><button type="button" className="inline-link" onClick={()=>setScannerOpen(true)}>Scan barcode</button>{barcode&&<span>Barcode: {barcode}</span>}</div>{productMessage&&<p className="product-message" role="status">{productMessage}</p>}
...
export default function MemorateApp() {
  ...
  const update = useAppUpdate(editorOpen || filterOpen || settings);
  ...
  function openNewNote() { feedScrollY.current = window.scrollY; setFeedTop(document.querySelector(".home-view")?.getBoundingClientRect().top ?? 60); setEditing(null); setEditorOpen(true); }
  ...
  return <div className="app-shell"><div className="app-container"><header className="app-header"><button className="brand" onClick={returnToFeed} aria-label="Memorate home"><BrandStar className="brand-symbol" /><span className="wordmark">memorate<span>.</span></span></button><button className={`icon-button settings-button ${settings ? "active" : ""}`} onClick={settings ? returnToFeed : openSettings} aria-label="Settings"><Settings2 size={20} /></button></header>
    {settings ? <main className="settings-view"><div className="screen-heading"><button className="back-button" onClick={returnToFeed}><ArrowLeft size={19} /> Collection</button><h1>Settings</h1></div><section className="settings-section"><h2>Appearance</h2><div className="setting-row"><div><strong>Theme</strong><p>Choose how Memorate looks.</p></div><FieldSelect label="Theme" value={preferences.theme} onChange={v => updatePrefs({ ...preferences, theme: v as UserPreferences["theme"] })} choices={[{ value: "system", label: "System" }, { value: "light", label: "Light" }, { value: "dark", label: "Dark" }, { value: "amoled", label: "AMOLED" }]} /></div><div className="setting-row"><div><strong>Default currency</strong><p>Used for new notes.</p></div><FieldSelect label="Default currency" value={preferences.defaultCurrency} onChange={v => updatePrefs({ ...preferences, defaultCurrency: v })} choices={CURRENCIES.map(c => ({ value: c, label: c }))} /></div></section>
      ...
      <DataSettings syncStatus={cloudSync.status} onSync={() => synchronize("manual")} onChange={async () => { await reload(); setPreferences(await repository.preferences()); }} onNotice={setNotice} /></main> : <><div className={`collection-layer ${selected ? "is-underneath" : ""}`} inert={!!selected} aria-hidden={!!selected} style={selected ? {top:feedTop} : undefined}><main className="home-view"><div className="home-heading"><div><p className="eyebrow">YOUR PERSONAL CATALOG</p><h1>All notes <span className="count">{filtered.length}</span></h1></div><button className="desktop-add primary-button" onClick={openNewNote}><Plus size={20} /> New note</button></div>
  ...
  <NoteEditor notes={notes} onOpenExisting={note=>{setEditorOpen(false);openNote(note.id);}} open={editorOpen} note={editing} categories={categories} currency={preferences.defaultCurrency} onClose={() => setEditorOpen(false)} onSave={saveNote} onAddCategory={addCategory} onDelete={setDeleteId} />
  ...
  {update.available && <div className="update-banner" role="status">Update available <button disabled={editorOpen || filterOpen || settings} onClick={update.apply}>{editorOpen ? "Save or close your note first" : settings ? "Return to collection to refresh" : "Refresh"}</button></div>}
