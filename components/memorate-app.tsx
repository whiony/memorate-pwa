...
import { ArrowLeft, CalendarDays, Camera, ChevronDown, ChevronLeft, ChevronRight, Download, Ellipsis, Minus, Plus, Search, Settings2, SlidersHorizontal, Star, Trash2, X } from "lucide-react";
...
function NoteEditor({ open, note, categories, currency, onClose, onSave, onAddCategory, onDelete }: {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      await onSave({ id: note?.id || newId(), title: title.trim(), rating, categoryId: categoryId || null, date, comment: comment.trim(), price: price === "" ? null : Number(price.replace(",", ".")), currency: selectedCurrency, photos, createdAt: note?.createdAt || now, updatedAt: now, syncState: "local" });
      onClose();
