...
import { SwipeCard, PullRefresh, SheetHandle } from "./mobile-interactions";
...
const compactDate = (date: string) => new Date(`${date}T12:00:00`).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "2-digit" });
...
function NoteEditor({ open, note, categories, currency, onClose, onSave, onAddCategory, onDelete }: {
  ...
  return <><Sheet open={open} onOpenChange={value => { if (!value) requestClose(); }}><SheetContent side="right" showCloseButton={false} className="editor-sheet"><SheetHandle onClose={requestClose} /><div className="sheet-top"><button className="icon-button" onClick={requestClose} aria-label="Close"><X /></button><SheetTitle>{note ? "Edit note" : "New note"}</SheetTitle><span className="sheet-top-spacer" /></div><SheetDescription className="sr-only">Save a memory with a title and optional details.</SheetDescription>
    <form id="note-form" onChangeCapture={() => setDirty(true)} onSubmit={save} className="editor-scroll"><label className="field-label" htmlFor="note-title">Title <span className="required">*</span></label><input id="note-title" className="title-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="What do you want to remember?" required maxLength={160} autoFocus />
...
function NoteCard({ note, category, onOpen }: { note: Note; category?: Category; onOpen: () => void }) {
  return <button className="note-card" onClick={onOpen} type="button"><div className="card-top"><span className="category-label"><span className="category-mark" style={{ background: category?.color || "#BCA9A2" }} />{category?.name || "Uncategorized"}</span><span className="card-date">{prettyDate(note.date)}</span></div><div className="card-body"><div className="note-card-content"><div className="card-main"><h2>{note.title}</h2><span className={`numeric-rating ${note.rating ? "" : "unrated"}`}>{note.rating ? <>{note.rating.toFixed(1)} <BrandStar className="rating-mark" /></> : "Not rated"}</span>{note.comment && <p className="card-preview">{note.comment}</p>}</div>{note.price != null && <span className="card-price">{money(note.price, note.currency)}</span>}</div>{note.photos[0] && <span className="card-photo-frame"><PhotoView blob={note.photos[0].blob} className="card-photo" alt="" /></span>}</div></button>;
}
...
export default function MemorateApp() {
  ...
  useLayoutEffect(() => { if (selectedId) window.scrollTo(0, 0); else if (feedReturnPending.current) { window.scrollTo(0, feedScrollY.current); feedReturnPending.current = false; } }, [selectedId]);
  const checkUpdate = update.check;
  const refresh = useCallback(async () => { try { if (await repository.syncMetadata()) { const result = await syncService.sync(); if (result.conflicts) setNotice("Conflicting edits preserved as recovered copies"); } await reload(); setPreferences(await repository.preferences()); await checkUpdate(); setNotice("Collection refreshed"); } catch { setNotice("Could not refresh. Your local notes are safe."); } }, [reload, checkUpdate]);
  useEffect(() => {
    let timer: ReturnType<typeof setTimeout> | undefined;
    let syncing = false;
    const originalRun = async () => { if (syncing || !navigator.onLine || !await repository.syncMetadata()) return; syncing = true; try { const result = await syncService.sync(); await reload(); setPreferences(await repository.preferences()); if (result.pending) timer = setTimeout(originalRun, 1500); if (result.conflicts) setNotice("Conflicting edits preserved as recovered copies"); } catch { /* Settings and pull-to-refresh expose recoverable sync errors. */ } finally { syncing = false; } };
    const changed = () => { clearTimeout(timer); if (!syncing) timer = setTimeout(originalRun, 1500); };
    const foreground = () => { if (document.visibilityState === "visible") changed(); };
    document.addEventListener("visibilitychange", foreground);
    window.addEventListener("online", changed); window.addEventListener("memorate-change", changed); void originalRun();
    return () => { clearTimeout(timer); document.removeEventListener("visibilitychange", foreground); window.removeEventListener("online", changed); window.removeEventListener("memorate-change", changed); };
  }, [reload]);
  const selected = notes.find(n => n.id === selectedId);
  function openNote(id: string) { feedScrollY.current = window.scrollY; setSelectedId(id); }
  function returnToFeed() { feedReturnPending.current = true; setSelectedId(null); }
  ...
  return <div className="app-shell"><div className="app-container"><header className="app-header"><button className="brand" onClick={() => { setSelectedId(null); setSettings(false); }} aria-label="Memorate home"><BrandStar className="brand-symbol" /><span className="wordmark">memorate<span>.</span></span></button><button className={`icon-button settings-button ${settings ? "active" : ""}`} onClick={() => { setSettings(!settings); setSelectedId(null); }} aria-label="Settings"><Settings2 size={20} /></button></header>
    {settings ? <main className="settings-view"><div className="screen-heading"><button className="back-button" onClick={() => setSettings(false)}><ArrowLeft size={19} /> Collection</button><h1>Settings</h1></div><section className="settings-section"><h2>Appearance</h2><div className="setting-row"><div><strong>Theme</strong><p>Choose how Memorate looks.</p></div><FieldSelect label="Theme" value={preferences.theme} onChange={v => updatePrefs({ ...preferences, theme: v as UserPreferences["theme"] })} choices={[{ value: "system", label: "System" }, { value: "light", label: "Light" }, { value: "dark", label: "Dark" }, { value: "amoled", label: "AMOLED" }]} /></div><div className="setting-row"><div><strong>Default currency</strong><p>Used for new notes.</p></div><FieldSelect label="Default currency" value={preferences.defaultCurrency} onChange={v => updatePrefs({ ...preferences, defaultCurrency: v })} choices={CURRENCIES.map(c => ({ value: c, label: c }))} /></div></section>
      ...
      <DataSettings onChange={async () => { await reload(); setPreferences(await repository.preferences()); }} onNotice={setNotice} /></main> : selected ? <Detail note={selected} category={categories.find(c => c.id === selected.categoryId)} onBack={returnToFeed} onEdit={() => { setEditing(selected); setEditorOpen(true); }} onDelete={() => setDeleteId(selected.id)} /> : <main className="home-view"><div className="home-heading"><div><p className="eyebrow">YOUR PERSONAL CATALOG</p><h1>All notes <span className="count">{filtered.length}</span></h1></div><button className="desktop-add primary-button" onClick={() => { setEditing(null); setEditorOpen(true); }}><Plus size={20} /> New note</button></div>
