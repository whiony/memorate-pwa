...
export function BarcodeCapture({notes,onUse,onExisting,onClose,currency='EUR'}:{currency?:string;notes:Note[];onUse:(draft:ProductDraft)=>void;onExisting:(note:Note)=>void;onClose:()=>void}) {
 ...
 async function lookup(input:string,skipDuplicate=false,detectedFormat?:string) {
  const parsed=((skipDuplicate||(!detectedFormat&&input===scanned.current?.barcode))&&scanned.current)||parseProductCode(input,detectedFormat||format);if(!parsed){setError('Enter a valid EAN, UPC or GTIN barcode, including its check digit.');return;}
 ...
 async function startCamera(){
  ...
  try{
   ...
   const scanner=await reader.decodeFromStream(media,video.current!,result=>{if(result&&session===generation.current){const value=parseProductCode(result.getText(),BarcodeFormat[result.getBarcodeFormat()]);if(value)void lookup(result.getText(),false,BarcodeFormat[result.getBarcodeFormat()]);}});
 ...
 return <Dialog open onOpenChange={open=>{if(!open)close();}}><DialogContent className="barcode-dialog" showCloseButton={false} onOpenAutoFocus={e=>{e.preventDefault();closeButton.current?.focus({preventScroll:true});}}><div className="scanner-heading"><DialogTitle>Scan product</DialogTitle><button type="button" ref={closeButton} className="icon-button" onClick={close} aria-label="Close scanner"><X/></button></div><DialogDescription>Scan a barcode or DataMatrix, or enter the code on the packaging.</DialogDescription>
  ...
  {!duplicate&&!notFound&&<button type="button" className="text-button" disabled={busy} onClick={()=>{stop();onUse(manualDraft());}}>Continue manually</button>}
